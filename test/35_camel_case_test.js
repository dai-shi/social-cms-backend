...
var mongodbUrl = process.env.TEST_MONGODB_URL || 'mongodb://localhost:27017/socialcmsdb_test';
...
describe('form login test', function() {
  ...
  it('should login as a user', function(done) {
    request.post('http://localhost:' + port + '/login/local', {
      ...
      assert.ok(response.body.user_id); //jshint ignore:line
      done();
    });
  });
...
describe('create post test', function() {
  ...
  it('should fail to post with created_time', function(done) {
    request.post('http://localhost:' + port + '/posts', {
      json: {
        created_time: 'xxx', //jshint ignore:line
        foo: 'bar'
      }
    }, function(error, response) {
      assert.equal(response.statusCode, 403);
  ...
  it('should post a new post with group scope', function(done) {
    request.post('http://localhost:' + port + '/posts', {
      json: {
        scope: [{
          group_id: 1 //jshint ignore:line
        }],
        foo: 'bar'
      }
    }, function(error, response) {
      assert.equal(response.statusCode, 200, response.body);
      assert.equal(response.body._id, basePostId + 1);
      done();
    });
  });
  ...
  it('should post a new post with user scope', function(done) {
    request.post('http://localhost:' + port + '/posts', {
      ...
      assert.equal(response.statusCode, 200, response.body);
      assert.equal(response.body._id, basePostId + 2);
      done();
    });
  });
  ...
  it('should post a new post with user/group scope', function(done) {
    request.post('http://localhost:' + port + '/posts', {
      ...
      assert.equal(response.statusCode, 200, response.body);
      assert.equal(response.body._id, basePostId + 3);
      done();
    });
  });
...
describe('get post test', function() {
  it('should get a post', function(done) {
    request.get({
      url: 'http://localhost:' + port + '/posts/' + basePostId,
      json: true
    }, function(error, response) {
      assert.equal(response.statusCode, 200, response.body);
      assert.equal(response.body.foo, 'bar');
      assert.equal(response.body._id, basePostId);
      done();
    });
  });
  ...
  it('should get a post', function(done) {
    request.get({
      url: 'http://localhost:' + port + '/posts/' + (basePostId + 1),
      json: true
    }, function(error, response) {
      assert.equal(response.statusCode, 200, response.body);
      assert.equal(response.body.foo, 'bar');
  ...
  it('should get a post', function(done) {
    request.get({
      url: 'http://localhost:' + port + '/posts/' + (basePostId + 2),
      json: true
    }, function(error, response) {
      assert.equal(response.statusCode, 200, response.body);
      assert.equal(response.body.foo, 'bar');
  ...
  it('should get a post', function(done) {
    request.get({
      url: 'http://localhost:' + port + '/posts/' + (basePostId + 3),
      json: true
    }, function(error, response) {
      assert.equal(response.statusCode, 200, response.body);
      assert.equal(response.body.foo, 'bar');
...
describe('update post test', function() {
  ...
  it('should get the updated post', function(done) {
    request.get({
      ...
      assert.equal(response.body.scope[0].group_id, 1); //jshint ignore:line
      done();
    });
  });
